import { listMyWorks } from "./listMyWorks";
...
describe("listMyWorks", () => {
  ...
  describe("happy path", () => {
    ...
    test("includes search param when filters.search is provided", async () => {
      ...
      await listMyWorks(0, 8, { search: "react" });
    ...
    test("appends multiple workType.name params for comma-separated workTypes", async () => {
      ...
      await listMyWorks(0, 8, { workTypes: "ARTICLE,DISSERTATION" });
    ...
    test("appends multiple workStatus.name params for comma-separated statuses", async () => {
      ...
      await listMyWorks(0, 8, { workStatus: "DRAFT,PUBLISHED" });
    ...
    test("appends sort param with desc when filters.order is 'desc'", async () => {
      ...
      await listMyWorks(0, 8, { order: "desc" });
    ...
    test("appends sort param with asc when filters.order is not 'desc'", async () => {
      ...
      await listMyWorks(0, 8, { order: "asc" });
    ...
    test("appends createdAt.goe for startDate", async () => {
      ...
      await listMyWorks(0, 8, { startDate: "2024-01-01" });
    ...
    test("appends createdAt.loe for endDate", async () => {
      ...
      await listMyWorks(0, 8, { endDate: "2024-12-31" });
    ...
    test("appends labels.name for each label in filters.labels", async () => {
      ...
      await listMyWorks(0, 8, { labels: ["JavaScript", "React"] });
    ...
    test("appends startDate and endDate plain params as well", async () => {
      api.get.mockResolvedValue({});

      await listMyWorks(0, 8, { startDate: "2024-01-01", endDate: "2024-12-31" });
    ...
    test("uses the LIST_MY_WORKS endpoint", async () => {
      api.get.mockResolvedValue({});

      await listMyWorks();

      const calledUrl = api.get.mock.calls[0][0];
      expect(calledUrl).toMatch(/^works\/list-my-works/);
    });

    test("combines multiple filters correctly", async () => {
      api.get.mockResolvedValue({});

      await listMyWorks(1, 12, {
  ...
  describe("error handling", () => {
    test("propagates error thrown by api.get", async () => {
      ...
      await expect(listMyWorks()).rejects.toThrow("Network error");
    });
    ...
    test("propagates 500 server error", async () => {
      ...
      await expect(listMyWorks()).rejects.toThrow("Internal Server Error");
    });
  });
});
